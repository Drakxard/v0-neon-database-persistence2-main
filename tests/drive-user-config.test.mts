...
import { openDriveUserConfig, readDriveUserConfig, sealDriveUserConfig } from "../lib/drive-user-config.ts"

test("User.Services cifra Drive sin exponer el refresh token", () => {
  ...
  try {
    ...
    assert.doesNotMatch(token, /refresh-super-secreto/)
    assert.deepEqual(openDriveUserConfig(token), config)
    const request = new Request("https://example.test", { headers: { "x-drive-config-token": token } })
