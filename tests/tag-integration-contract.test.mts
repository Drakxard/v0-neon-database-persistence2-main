...
test("PDF.js traduce la seleccion junto al texto y permite editar el prompt", () => {
  ...
  assert.match(pdfJs, /inscreenApiFetch\("\/api\/pdf-translate"/)
  assert.match(pdfJs, /x-inscreen-config-token/)
  ...
  assert.match(interceptor, /pathname === "\/api\/pdf-translate"/)
  assert.match(interceptor, /getReadyInscreenConfigToken/)
  assert.match(proxy, /pathname === "\/api\/pdf-translate"[\s\S]*?NextResponse\.next\(\)/)
})

test("PDF.js autoguarda anotaciones y exporta paginas JPG en ZIP", () => {
  const html = source("public/pdfjs/web/viewer.html")
  const css = source("public/pdfjs/web/viewer-custom.css")
  const pdfJs = source("public/pdfjs/web/viewer-custom.js")
  assert.doesNotMatch(html, /id="syncButton"|id="secondarySyncButton"/)
  assert.match(html, /id="pagesZipButton"[\s\S]*?<svg/)
  assert.match(html, /id="secondaryPagesZipButton"[\s\S]*?Paginas JPG \(\.zip\)/)
  assert.match(css, /#pagesZipButton\.pdfjs-custom-pages-zip-button[\s\S]*?min-width: 32px/)
  assert.match(pdfJs, /AUTOSYNC_DEBOUNCE_MS = 2000/)
  assert.match(pdfJs, /annotationStorage\.serializable\?\.hash/)
  assert.match(pdfJs, /window\.setInterval\(monitorAnnotationChanges, 400\)/)
  assert.match(pdfJs, /getAnnotationContentHash\(\) === syncingHash/)
  assert.match(pdfJs, /buildStoredZip/)
  assert.match(pdfJs, /getViewport\(\{ scale: 2 \}\)/)
  assert.match(pdfJs, /"image\/jpeg",\s*0\.92/)
  assert.match(pdfJs, /pagina-\$\{String\(pageNumber\)\.padStart\(digits, "0"\)\}\.jpg/)
  assert.match(pdfJs, /\$\{sourceName\}-paginas\.zip/)
})

test("las herramientas PDF separan activacion y configuracion por pulsacion larga", () => {
  const pdfJs = source("public/pdfjs/web/viewer-custom.js")
  const css = source("public/pdfjs/web/viewer-custom.css")
  assert.match(pdfJs, /EDITOR_TOOL_LONG_PRESS_MS = 500/)
  assert.match(pdfJs, /editorHighlightParamsToolbar/)
...
test("el visor muestra compactos los accesos con link de la materia", () => {
  ...
  assert.match(viewer, /getSubjectShortcutUrl\(shortcut, sectionKey\)/)
  assert.match(viewer, /filter\(\(shortcut\) => Boolean\(shortcut\.effectiveUrl\?\.trim\(\)\)\)/)
