...
test("el visor PDF.js captura y persiste varias regiones por tag", () => {
  ...
  assert.match(pdfJs, /crop\.style\.width = `\$\{sourceWidth \/ outputScale\}px`/)
  assert.match(pdfJs, /presentationScale: 1\.5/)
  assert.match(pdfJs, /eventBus\.on\("scalechanging", handleViewerScaleChanging\)/)
  assert.match(pdfJs, /currentScaleValue = "1\.5"/)
  assert.doesNotMatch(pdfJs, /Math\.min\(3, Math\.max\(0\.5/)
...
test("el visor exporta rangos inclusivos como un PDF rasterizado descargable", () => {
  const pdfJs = source("public/pdfjs/web/viewer-custom.js")
  ...
  assert.match(pdfJs, /buildRasterizedPageRangePdf/)
  assert.match(pdfJs, /getViewport\(\{ scale: 2 \}\)/)
  assert.match(pdfJs, /canvas\.toBlob/)
  assert.match(pdfJs, /"image\/jpeg", quality/)
  assert.match(pdfJs, /outputDoc\.embedJpg/)
  assert.match(pdfJs, /_paginas-\$\{startPage\}-\$\{endPage\}/)
  assert.match(pdfJs, /URL\.createObjectURL/)
  assert.match(pdfJs, /card\.dataset\.pageNumber/)
