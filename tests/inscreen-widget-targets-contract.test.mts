...
test("Drive separa fijos, crea shortcuts y publica solo carpetas sincronizadas no fijas", () => {
  ...
  assert.match(workspace, /repairLocalWidgetTargetSyncManifest/)
  assert.match(workspace, /catalogIds\.has\(item\.target\.subjectId\)/)
  assert.match(workspace, /catalogIds\.has\(key\.slice\(0, key\.lastIndexOf\(":"\)\)\)/)
