...
test("el proveedor expone GET protegidos y conserva los TXT como contenido", () => {
  ...
  assert.match(provider, /listInscreenProviderStages/)
  assert.match(provider, /metadata\.metadata\["subject-id"\]/)
  assert.doesNotMatch(provider, /getSubjectById/)
  assert.match(provider, /getDateKeyInTimeZone\(new Date\(object\.lastModified\)\)/)
  assert.match(provider, /return jsonResponse\(\{ ok: true, archivos \}\)/)
  assert.match(provider, /return jsonResponse\(\{ ok: false, archivos: \[\] \}/)
