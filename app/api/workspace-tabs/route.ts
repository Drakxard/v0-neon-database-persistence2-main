...
export async function GET() {
  ...
  if (!isLocalStorageMode()) {
    return NextResponse.json({ workspaceTabs: {}, activeWorkspaceTabId: "main", customSubjects: {} })
  }
...
export async function POST(request: Request) {
  ...
  if (!isLocalStorageMode()) {
    return NextResponse.json({ error: "Workspace tabs only support local mode." }, { status: 400 })
  }
