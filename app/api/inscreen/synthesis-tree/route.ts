...
export async function GET(request: Request) {
  return withInscreenUserConfig(request, async () => {
    try {
      ...
      const snapshot = await readSynthesisTree()
...
export async function PUT(request: Request) {
  return withInscreenUserConfig(request, async () => {
    try {
      ...
      const body = await request.json().catch(() => null) as { tree?: unknown; etag?: unknown; force?: unknown } | null
      if (!body || body.tree === undefined) return Response.json({ error: "Falta el árbol de Síntesis." }, { status: 400 })
      const result = await writeSynthesisTree(body.tree, typeof body.etag === "string" ? body.etag : null, body.force === true)
