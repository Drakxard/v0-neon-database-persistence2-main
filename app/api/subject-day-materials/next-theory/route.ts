...
export async function GET(request: Request) {
  try {
      ...
      WHERE subject_id = ${subjectId}
        ...
        AND (
          container_id IS NULL OR EXISTS (
            ...
            WHERE container.id = subject_day_materials.container_id AND container.kind = 'theory'
          )
        )
    ...
    console.error("GET /api/subject-day-materials/next-theory error:", error)
    if (isMissingTable(error)) {
      return NextResponse.json({ material: null, previousFeaturedEntry: null })
    }
    return NextResponse.json({ error: "Failed to fetch next theory material" }, { status: 500 })
  }
}
